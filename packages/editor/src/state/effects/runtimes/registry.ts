...
// Cache for loaded runtime factories to avoid re-importing
const runtimeCache = new Map<RuntimeType, RuntimeFactory>();
...
export async function loadRuntime(runtimeType: RuntimeType): Promise<RuntimeFactory> {
  // Check cache first
  const cached = runtimeCache.get(runtimeType);
  if (cached) {
    return cached;
  }
  ...
  try {
    ...
    runtimeCache.set(runtimeType, factory);
    
    return factory;
  } catch (error) {
    throw new Error(`Failed to load runtime ${runtimeType}: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
