 ...
 *   'moduleEnd'
 * ];
 * const result = parsePos(code); // { x: 10, y: 20 }
 * ```
 */
export default function parsePos(code: string[]): PosParseResult | undefined {
	let foundPos: PosParseResult | undefined = undefined;

	for (const line of code) {
		// Match semicolon comment lines with @pos directive followed by arguments
		const commentMatch = line.match(/^\s*;\s*@(\w+)\s+(.*)/);
		if (commentMatch && commentMatch[1] === 'pos') {
			...
			if (isNaN(x) || isNaN(y) || tokens[0] !== x.toString() || tokens[1] !== y.toString()) {
				return undefined;
			}
