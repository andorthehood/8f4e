...
describe('compileAndUpdateMemory multi-memory regions', () => {
	let compileAndUpdateMemory: typeof compileAndUpdateMemoryType;
	...
	it('creates region memories and instantiates modules with all memory imports', async () => {
		const result = await compileAndUpdateMemory(
			[
				{
					code: ['module samples', '#region sampleMemory', 'int8[] values 4 7', 'moduleEnd'],
				},
				{
					code: ['module reader', 'int result', 'push &result', 'push &samples:values', 'load8s', 'store', 'moduleEnd'],
				},
			],
			{ memoryRegions: ['sampleMemory'] }
		);

		const sampleMemory = result.memoryRefsByRegion?.sampleMemory;
		...
		expect(sampleMemory).toBeInstanceOf(WebAssembly.Memory);
		expect(result.requiredMemoryBytesByRegion).toEqual({ sampleMemory: 8 });
		expect(new DataView(sampleMemory!.buffer).getInt8(values.byteAddress)).toBe(7);

		const { instance } = await WebAssembly.instantiate(result.codeBuffer, {
			js: {
				memory: result.memoryRef,
				...result.memoryRefsByRegion,
			},
		});
		(instance.exports.init as CallableFunction)();
		(instance.exports.cycle as CallableFunction)();

		expect(new DataView(result.memoryRef.buffer).getInt32(resultMemory.byteAddress, true)).toBe(7);
	});
});
