...
| 365 | Unify editor directive parsing and alias normalization | 🟢 | 2-4h | 2026-04-03 | Editor directive parsing is currently split across multiple overlapping paths (`parseDirectiveComment`, `parseEditorDirectives`, and `parseBlockDirectives`), and plugin-alias normalization must be kept in sync across more than one pass. This already caused inline `@watch` and `@w` behavior to drift between direct parsing and cached block-directive resolution. |
