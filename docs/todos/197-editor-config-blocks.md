...
`packages/editor/packages/editor-state/src/features/code-blocks/utils/codeParsers/getBlockType.ts` recognizes config blocks, `packages/editor/packages/editor-state/src/features/config-compiler/extractConfigBody.ts` extracts the config type, and `packages/editor/packages/editor-state/src/features/config-compiler/effect.ts` compiles config blocks into state. The config blocks are combined in `packages/editor/packages/editor-state/src/features/config-compiler/combineConfigBlocks.ts`, which currently only supports the `project` config type (see `SUPPORTED_CONFIG_TYPES` there). The schema lives in `packages/editor/packages/editor-state/src/features/config-compiler/configSchema.ts`, and the output type is defined in `packages/editor/packages/editor-state/src/features/config-compiler/types.ts` as `ConfigObject`.
