...
describe('inter-module references - element min', () => {
	test('resolves element min reference for signed int32 in declaration default (!module.memory)', () => {
		const modules = [
			{ code: ['module sourceModule', 'int[] buffer 10 0', 'moduleEnd'] },
			{ code: ['module targetModule', 'int minValue !sourceModule.buffer', 'moduleEnd'] },
		];
		...
		// minValue should be set to the element min value of buffer (signed int32)
	...
	test('resolves element min reference for signed int32', () => {
		...
		expect(targetModule).toBeDefined();
		expect(targetModule.memoryMap['minValue']).toBeDefined();
		// minValue should be set to the element min value (signed int32)
		expect(targetModule.memoryMap['minValue'].default).toBe(-2147483648);
	});
	...
	test('resolves element min reference for float32', () => {
		const modules = [
			...
			{ code: ['module targetModule', 'float minValue !sourceModule.data', 'moduleEnd'] },
		];
		...
		expect(targetModule).toBeDefined();
		expect(targetModule.memoryMap['minValue']).toBeDefined();
		// minValue should be set to the lowest finite float32 value
		expect(targetModule.memoryMap['minValue'].default).toBe(-3.4028234663852886e38);
	});
	...
	test('resolves element min reference for signed int16', () => {
		...
		expect(targetModule).toBeDefined();
		expect(targetModule.memoryMap['minValue']).toBeDefined();
		// minValue should be set to the element min value (signed int16)
		expect(targetModule.memoryMap['minValue'].default).toBe(-32768);
	});
	...
	test('resolves element min reference for unsigned int16', () => {
		...
		expect(targetModule).toBeDefined();
		expect(targetModule.memoryMap['minValue']).toBeDefined();
		// minValue should be set to the element min value (unsigned int16 -> 0)
	...
	test('resolves element min reference for signed int8', () => {
		...
		expect(targetModule).toBeDefined();
		expect(targetModule.memoryMap['minValue']).toBeDefined();
		// minValue should be set to the element min value (signed int8)
		expect(targetModule.memoryMap['minValue'].default).toBe(-128);
	});
	...
	test('resolves element min reference for unsigned int8', () => {
		...
		expect(targetModule).toBeDefined();
		expect(targetModule.memoryMap['minValue']).toBeDefined();
		// minValue should be set to the element min value (unsigned int8 -> 0)
		expect(targetModule.memoryMap['minValue'].default).toBe(0);
	});
	...
	test('resolves element min reference in init instruction', () => {
		const modules = [
			{ code: ['module sourceModule', 'int[] buffer 10 0', 'moduleEnd'] },
			{ code: ['module targetModule', 'int minValue', 'init minValue !sourceModule.buffer', 'moduleEnd'] },
		];
	...
	test('rejects multi-dot element min reference', () => {
		const modules = [
			...
			{ code: ['module targetModule', 'int minValue !sourceModule.buffer.extra', 'moduleEnd'] },
		];
	...
	test('rejects multi-dot element min reference in init instruction', () => {
		const modules = [
			{ code: ['module sourceModule', 'int[] buffer 5 0', 'moduleEnd'] },
			{ code: ['module targetModule', 'int minValue', 'init minValue !sourceModule.buffer.extra', 'moduleEnd'] },
		];
	...
	test('throws error for unknown module in element min reference', () => {
		const modules = [
			{ code: ['module sourceModule', 'int[] buffer 5 0', 'moduleEnd'] },
			{ code: ['module targetModule', 'int minValue !unknownModule.buffer', 'moduleEnd'] },
		];
	...
	test('throws error for unknown memory in element min reference', () => {
		const modules = [
			{ code: ['module sourceModule', 'int[] buffer 5 0', 'moduleEnd'] },
			{ code: ['module targetModule', 'int minValue !sourceModule.unknownMemory', 'moduleEnd'] },
		];
	...
	test('module dependency sorting works with element min references', () => {
		const modules = [
			{ code: ['module dependentModule', 'int minValue !baseModule.buffer', 'moduleEnd'] },
		...
		expect(dependentModule.memoryMap['minValue'].default).toBe(-2147483648);
	});

	test('works with multiple element min references', () => {
		const modules = [
			{ code: ['module moduleA', 'int[] bufferA 10 0', 'moduleEnd'] },
			{ code: ['module moduleB', 'float[] bufferB 5 0.0', 'moduleEnd'] },
			{
				code: ['module moduleC', 'int minA !moduleA.bufferA', 'float minB !moduleB.bufferB', 'moduleEnd'],
			},
		];
		...
		expect(moduleC.memoryMap['minA'].default).toBe(-2147483648);
		expect(moduleC.memoryMap['minB'].default).toBe(-3.4028234663852886e38);
	});

	test('combines element min with other inter-module references', () => {
		const modules = [
		...
		// Verify minValue holds element min
		expect(targetModule.memoryMap['minValue'].default).toBe(-2147483648);
