...
- `src/benchmarks/opcode-size/`: benchmark cases for measuring emitted opcode counts.

## Hosted Examples

- Modules are deployed to Cloudflare R2 at `https://static.8f4e.com/example-modules/` with `registry.json`.
- Projects are deployed to Cloudflare R2 at `https://static.8f4e.com/example-projects/` with `registry.json`.
