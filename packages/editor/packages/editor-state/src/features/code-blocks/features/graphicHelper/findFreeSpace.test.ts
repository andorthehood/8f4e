...
describe('findFreeSpace', () => {
	...
	it('returns (0, 0) when existing block does not overlap candidate at (0, 0)', () => {
		const blocks = [createMockCodeBlock({ gridX: 20, gridY: 0, gridWidth: 10, gridHeight: 5 })];
		expect(findFreeSpace(blocks, 10, 5)).toEqual({ x: 0, y: 0 });
	});

	it('skips rows blocked at x=0 and returns first free row', () => {
		// Block occupies gridY 0..4 at x=0 (gridHeight 5, gridWidth 10)
		const blocks = [createMockCodeBlock({ gridX: 0, gridY: 0, gridWidth: 10, gridHeight: 5 })];
		expect(findFreeSpace(blocks, 10, 5)).toEqual({ x: 0, y: 5 });
	});

	it('finds first free row when consecutive rows are blocked', () => {
		const blocks = [
			createMockCodeBlock({ gridX: 0, gridY: 0, gridWidth: 10, gridHeight: 3 }),
			createMockCodeBlock({ gridX: 0, gridY: 3, gridWidth: 10, gridHeight: 4 }),
		];
	...
	it('correctly detects overlap when target is smaller than existing block', () => {
		const blocks = [createMockCodeBlock({ gridX: 0, gridY: 0, gridWidth: 50, gridHeight: 20 })];
		expect(findFreeSpace(blocks, 5, 3)).toEqual({ x: 0, y: 20 });
	});

	it('does not consider blocks positioned far to the right as blocking x=0', () => {
		const blocks = [createMockCodeBlock({ gridX: 100, gridY: 0, gridWidth: 10, gridHeight: 5 })];
		expect(findFreeSpace(blocks, 10, 5)).toEqual({ x: 0, y: 0 });
	});

	it('handles blocks at negative gridY gracefully', () => {
		// Block at negative gridY should not block y=0 placement
		const blocks = [createMockCodeBlock({ gridX: 0, gridY: -5, gridWidth: 10, gridHeight: 3 })];
		expect(findFreeSpace(blocks, 10, 5)).toEqual({ x: 0, y: 0 });
	});

	it('uses gridHeight (not code.length) so gap rows are accounted for', () => {
		// Block with gridHeight 13 due to a @plot gap (5 code lines + 8 gap rows)
		const blocks = [createMockCodeBlock({ gridX: 0, gridY: 0, gridWidth: 10, gridHeight: 13 })];
		expect(findFreeSpace(blocks, 10, 5)).toEqual({ x: 0, y: 13 });
	});
	...
	it('places multiple blocks without overlap when called sequentially', () => {
		...
		blocks.push(createMockCodeBlock({ gridX: pos1.x, gridY: pos1.y, gridWidth: w, gridHeight: h }));

		const pos2 = findFreeSpace(blocks, w, h);
		expect(pos2).toEqual({ x: 0, y: 5 });
		blocks.push(createMockCodeBlock({ gridX: pos2.x, gridY: pos2.y, gridWidth: w, gridHeight: h }));
...
describe('hasCollision', () => {
	...
	it('returns true when candidate overlaps an existing block', () => {
		const blocks = [createMockCodeBlock({ gridX: 5, gridY: 5, gridWidth: 10, gridHeight: 5 })];
	...
	it('returns true for partial overlap', () => {
		const blocks = [createMockCodeBlock({ gridX: 5, gridY: 0, gridWidth: 10, gridHeight: 5 })];
