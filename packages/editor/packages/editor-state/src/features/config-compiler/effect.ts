...
/**
 * Effect that compiles projectConfig blocks and applies the resulting configuration to state.
 * All projectConfig blocks are combined into a single source for full schema validation.
...
export default function configEffect(store: StateManager<State>, events: EventDispatcher): void {
	...
	/**
	 * Rebuilds the config from all projectConfig blocks and applies it to state.
	 * All projectConfig blocks are combined and compiled as a single source for full schema validation.
	...
	async function rebuildConfig(): Promise<void> {
		...
		// Combine all projectConfig blocks once
	...
	store.subscribe('graphicHelper.selectedCodeBlock.code', () => {
		...
		if (state.graphicHelper.selectedCodeBlock?.blockType !== 'projectConfig') {
			return;
		}
		rebuildConfig();
	});
	store.subscribe('graphicHelper.selectedCodeBlockForProgrammaticEdit.code', () => {
		if (state.graphicHelper.selectedCodeBlockForProgrammaticEdit?.blockType !== 'projectConfig') {
			return;
		}
		rebuildConfig();
	});
