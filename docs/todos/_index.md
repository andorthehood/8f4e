...
| 344 | Move identifier existence validation into semantic pass and shrink codegen validation | 🟡 | 4-8h | 2026-03-28 | Instruction compilers still discover undeclared const/memory/module errors during codegen even though those failures should be owned by the semantic pass. |
| 345 | Tighten tokenizer-to-compiler contract with typed AST lines | 🟡 | 1-2d | 2026-03-28 | The tokenizer/runtime boundary is still enforced mostly by runtime validation and comments, while the TypeScript AST contract remains broad enough to represent impossible instruction/argument combinations. |
| 346 | Split semantic normalization into instruction-specific files | 🟡 | 4-8h | 2026-03-28 | `normalizeCompileTimeArguments.ts` is accumulating instruction-specific folding and existence-validation rules in one growing file instead of following the semantic ownership split already used elsewhere. |
| 336 | Move identifier reference classification into tokenizer | 🟡 | 6-10h | 2026-03-27 | The compiler still re-parses identifier-shaped argument strings to determine whether they are memory, constant, address, module-address, pointer-dereference, or metadata-query references instead of receiving that classification directly from the AST parser. |
| 337 | Add structured address and query extraction to tokenizer | 🟡 | 4-8h | 2026-03-27 | The compiler still reconstructs address/query syntax details such as module ids, memory ids, start/end address markers, and pointee flags from raw identifier strings instead of receiving those extracted fields directly from the AST parser. |
