...
| 288 | Add split hexadecimal memory defaults | 🟡 | 4-8h | 2026-03-08 | Allow memory declarations like `int foo 0xA8 0xFF` and `int 0xA8 0xFF`, combining hex-byte chunks left-to-right into one integer default with right-side zero padding |
| 289 | Add decimal split-byte memory defaults | 🟡 | 4-8h | 2026-03-09 | Allow declarations like `int foo 32 64` and `int 32 64`, treating 2+ byte-sized integer literals as left-to-right split-byte defaults while preserving existing one-literal behavior |
| 290 | Add constants to split-byte memory defaults and reserve constant-style identifiers | 🟡 | 1-2d | 2026-03-09 | Allow constants in split-byte declarations like `int foo HI LO` and `int HI LO`, reject constant-style memory names, and add comprehensive language documentation |
| 291 | Add int64 support across compiler, runtime, and docs | 🟡 | 2-4d | 2026-03-09 | Add signed `int64` declarations and `i64` compiler/runtime support with 8-byte alignment, precision-safe host handling, and comprehensive documentation |
| 292 | Refactor error systems and document syntax vs compiler error boundaries | 🟡 | 1-2d | 2026-03-09 | Rename `errors.ts`, centralize syntax error messages, define the phase-based syntax/compiler split, and document the policy for future contributors and agents |
| 293 | Add separate color for non-decimal literal base prefixes | 🟡 | 4-8h | 2026-03-09 | Add a dedicated color-scheme entry for `0x`/`0b` base prefixes and update syntax highlighting to recognize them separately from the digits |
| 295 | Unify code render rows and width derivation | 🟢 | 2-4h | 2026-03-09 | Extract one shared rendered-row builder so `codeToRender` and code-block width both use the exact same line-number/tab-expanded row model |
| 297 | Add url editor directive for clickable links | 🟡 | 4-8h | 2026-03-12 | Add `; @url https://...` as an editor-only directive that renders a clickable link and opens it in a new browser tab |
| 298 | Move group directive under central directive system | 🟡 | 4-8h | 2026-03-13 | Move `; @group` onto the shared editor-directive parsing path, keep group metadata derivation under `features/directives/group/`, and ignore malformed directives with missing required args |
| 299 | Move favorite directive under central directive system | 🟡 | 2-4h | 2026-03-13 | Move `; @favorite` onto the shared editor-directive parsing path and keep favorites derivation consuming directive-owned metadata instead of a separate parser path |
| 300 | Extract directive editing into shared feature | 🟡 | 4-8h | 2026-03-13 | Separate directive source-editing mechanics from directive semantics, centralize shared upsert/remove/replace helpers, and keep action features like `clearDebugProbes` as thin callers |
| 302 | Add jump editor directive for code block navigation | 🟡 | 4-8h | 2026-03-14 | Add `; @jump <code-block-id>` as an editor-only clickable directive that jumps the viewport to the matching code block by id |
