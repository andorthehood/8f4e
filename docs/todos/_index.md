...
| 095 | Split Loader/Save Effects into Dedicated Modules | 🟡 | 1-2d | 2025-11-17 | `loader.ts` and `save.ts` currently mix editor settings, project import, and export logic; delete both and replace them with `projectImport.ts`, `projectExport.ts`, and `editorSettings.ts` imported directly by `index.ts` |
