---
title: 'TODO: Move shared compiler constants to compiler-spec'
priority: Medium
effort: 2-4h
created: 2026-05-06
status: Completed
...
# TODO: Move shared compiler constants to compiler-spec
...
`WASM_MEMORY_PAGE_SIZE` is now available through `@8f4e/compiler-spec`, but other shared constants may still require callers to import from lower-level implementation packages or duplicate values locally. That leaks package internals into consumers and makes it easier for constants to drift.
...
Move or re-export those constants from `@8f4e/compiler-spec` when they are stable public contract values. Keep constants inside implementation packages when they are genuinely private to code generation, parsing, rendering, or runtime internals.
...
- Export public contract constants from `@8f4e/compiler-spec`.
- Update consumers to import from `@8f4e/compiler-spec` instead of lower-level packages.
...
- [x] Shared compiler contract constants are exported from `@8f4e/compiler-spec`.
...
- `packages/compiler-spec` - Public shared compiler contract constants.
