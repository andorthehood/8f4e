...
| 147 | Split Memory Instruction Into int/float With Shared Helpers | 🟡 | 4-6h | 2025-12-25 | Split `memory.ts` into `int.ts`/`float.ts`, add shared helpers, and split `memory.test.ts` into `int.test.ts`/`float.test.ts` |
