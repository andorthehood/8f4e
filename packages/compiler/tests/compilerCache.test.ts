...
describe('compiler AST cache', () => {
	test('returns a cache and reuses unchanged module AST entries', () => {
		const first = compile(
			{ entries: { main: [moduleWithDefault('first', 1), moduleWithDefault('second', 2)] } },
			options
		);
		const firstModuleAst = first.cache.ast.entries.get('entry:main:module:0')?.ast;
		const secondModuleAst = first.cache.ast.entries.get('entry:main:module:1')?.ast;
		...
		expect(second.cache.ast.entries.get('entry:main:module:0')?.ast).toBe(firstModuleAst);
		expect(second.cache.ast.entries.get('entry:main:module:1')?.ast).toBe(secondModuleAst);
	...
	test('reparses only changed module AST entries', () => {
		...
		const second = compile(
			{ entries: { main: [moduleWithDefault('first', 3), moduleWithDefault('second', 2)] } },
			options,
			first.cache
		);

		expect(second.cache.ast.entries.get('entry:main:module:0')?.ast).not.toBe(firstModuleAst);
		expect(second.cache.ast.entries.get('entry:main:module:1')?.ast).toBe(secondModuleAst);
	...
	test('caches function AST entries separately from module AST entries', () => {
		const first = compile(
			{ entries: { main: [moduleWithDefault('app', 0)] }, functions: [functionWithLiteral(1)] },
			options
		);
		const moduleAst = first.cache.ast.entries.get('entry:main:module:0')?.ast;
		const functionAst = first.cache.ast.entries.get('function:0')?.ast;

		const second = compile(
			{ entries: { main: [moduleWithDefault('app', 0)] }, functions: [functionWithLiteral(2)] },
			options,
			first.cache
		);

		expect(second.cache.ast.entries.get('entry:main:module:0')?.ast).toBe(moduleAst);
	...
	test('hashes macro-expanded source and line metadata', () => {
		...
		const first = compile({ entries: { main: modules }, macros: macros }, options);
		const moduleAst = first.cache.ast.entries.get('entry:main:module:0')?.ast;

		const second = compile({ entries: { main: modules }, macros: macros }, options, first.cache);
		const cachedAst = second.cache.ast.entries.get('entry:main:module:0')?.ast;
		...
		const third = compile({ entries: { main: modules }, macros: changedMacros }, options, first.cache);

		expect(third.cache.ast.entries.get('entry:main:module:0')?.ast).not.toBe(moduleAst);
	...
	test('produces equivalent output with and without a reused cache', () => {
		...
		const first = compile({ entries: { main: modules } }, options);
		const fresh = compile({ entries: { main: modules } }, options);
		const cached = compile({ entries: { main: modules } }, options, first.cache);
