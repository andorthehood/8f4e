...
| 094 | Handle Large Binary Assets with OPFS | 🟡 | 1-2d | 2025-11-17 | Binary asset imports in `src/storage-callbacks.ts:158` always serialize the uploaded file into a base64 data URL via `arrayBufferToDataUrl`. That works for icons and other tiny... |
| 146 | Investigate index arithmetic support | 🟡 | 2-4 hours | 2025-12-25 | 8f4e currently only supports byte-level arithmetic for memory addresses. This makes index-style arithmetic (like C pointer/index math) cumbersome, error-prone, and inconsistent... |
...
| 210 | Add WebMCP-based MCP server integration to the 8f4e editor | 🟡 | 2-4d | 2026-02-07 | The editor currently has no built-in MCP server integration path for browser-hosted workflows. That limits direct tool interoperability from within the 8f4e editor experience an... |
