...
import * as codeBlockNavigationModule from '../code-blocks/features/codeBlockNavigation/effect';
...
vi.mock('../code-blocks/features/codeBlockNavigation/effect', () => ({
...
describe('demoModeNavigation', () => {
	...
	it('should select a random code block on init when none is selected', () => {
		...
		expect(state.graphicHelper.codeBlocks.includes(state.graphicHelper.selectedCodeBlock!)).toBe(true);
	});
