...
export function getError(
	...
	switch (code) {
		...
		case ErrorCode.MISSING_ARGUMENT:
			return { stage: 'compiler', code, message: `Missing argument. (${code})`, line, context };
		...
		case ErrorCode.EXPECTED_IDENTIFIER:
			return { stage: 'compiler', code, message: `Expected identifier, got a value. (${code})`, line, context };
		case ErrorCode.UNRECOGNISED_INSTRUCTION:
			return { stage: 'compiler', code, message: `Unrecognised instruction. (${code})`, line, context };
		case ErrorCode.EXPECTED_VALUE:
			return { stage: 'compiler', code, message: `Expected value, got an identifier. (${code})`, line, context };
		case ErrorCode.MISSING_MODULE_ID:
			return { stage: 'compiler', code, message: `Missing module ID. (${code})`, line, context };
		...
		case ErrorCode.MISSING_BLOCK_START_INSTRUCTION:
			return { stage: 'compiler', code, message: `Missing block start instruction. (${code})`, line, context };
		case ErrorCode.DIVISION_BY_ZERO:
			return { stage: 'compiler', code, message: `Possible division by zero. (${code})`, line, context };
		case ErrorCode.MISSING_FUNCTION_ID:
			return { stage: 'compiler', code, message: `Missing function ID. (${code})`, line, context };
		case ErrorCode.INVALID_FUNCTION_SIGNATURE:
			return {
				stage: 'compiler',
				code,
				message: `Invalid function signature. Parameters and returns must be "int", "float", or "float64". (${code})`,
				line,
				context,
			};
		...
		case ErrorCode.TYPE_MISMATCH:
			return { stage: 'compiler', code, message: `Type mismatch. (${code})`, line, context };
		...
		case ErrorCode.UNDEFINED_FUNCTION:
			return { stage: 'compiler', code, message: `Undefined function. (${code})`, line, context };
		case ErrorCode.PARAM_AFTER_FUNCTION_BODY:
			return {
				stage: 'compiler',
				code,
				message: `Parameter declarations must come immediately after the function declaration, before any other instructions or local variable declarations. (${code})`,
				line,
				context,
			};
		...
		case ErrorCode.MIXED_FLOAT_WIDTH:
			return {
				stage: 'compiler',
				code,
				message: `Mixed float widths: arithmetic operations require all float operands to have the same width (float32 or float64). (${code})`,
				line,
				context,
			};
		...
		case ErrorCode.SPLIT_HEX_MIXED_TOKENS:
			return {
				stage: 'compiler',
				code,
				message: `Split-byte default values must consist entirely of byte-resolving tokens: integer literals (0–255), literal-only * or / expressions that fold to an integer in that range, or constant-style identifiers. Memory references and non-byte-resolving forms are not allowed in split-byte sequences. (${code})`,
				line,
				context,
			};
		case ErrorCode.CONSTANT_NAME_AS_MEMORY_IDENTIFIER:
			return {
				stage: 'compiler',
				code,
				message: `Constant-style identifiers (all-uppercase) are reserved for constants and cannot be used as memory allocation names. (${code})`,
				line,
				context,
			};
		case ErrorCode.RESERVED_MEMORY_IDENTIFIER:
			return {
				stage: 'compiler',
				code,
				message: `Reserved identifier cannot be used as a memory allocation name${details?.identifier ? `: ${details.identifier}` : ''}. (${code})`,
				line,
				context,
			};
		...
		case ErrorCode.LOCAL_NAME_COLLISION_WITH_MEMORY:
			return {
				stage: 'compiler',
				code,
				message: `Local variable name collides with a memory declaration${details?.identifier ? `: ${details.identifier}` : ''}. Local names must be unique within the namespace. (${code})`,
				line,
				context,
			};
		case ErrorCode.DUPLICATE_IDENTIFIER:
			return {
				stage: 'compiler',
				code,
				message: `Duplicate identifier${details?.identifier ? `: ${details.identifier}` : ''}. Module and function IDs must be unique. (${code})`,
				line,
				context,
			};
		case ErrorCode.LAYOUT_DEPENDENT_CONSTANT:
			return {
				stage: 'compiler',
				code,
				message: `Constants cannot depend on memory layout or memory metadata${details?.identifier ? `: ${details.identifier}` : ''}. (${code})`,
				line,
				context,
			};
		case ErrorCode.UNKNOWN_ERROR:
		default:
			return { stage: 'compiler', code, message: `Unknown error (${code})`, line, context };
	}
}
