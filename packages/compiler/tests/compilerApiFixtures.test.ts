...
describe('compiler API fixture helpers', () => {
	test('expands prototype memory shapes into consuming modules', () => {
...
`,
		...
		const oscA = compileResult.compiledModules.oscA;
		const oscB = compileResult.compiledModules.oscB;

		expect(Object.keys(oscA.memoryMap)).toEqual(['phase', 'frequency', 'localA']);
		expect(Object.keys(oscB.memoryMap)).toEqual(['localB', 'phase', 'frequency']);
		expect(oscA.memoryMap.phase.byteAddress).not.toBe(oscB.memoryMap.phase.byteAddress);
		expect(oscA.memoryMap.frequency.default).toBe(440);
		expect(oscB.memoryMap.frequency.default).toBe(440);
		expect(
			oscA.ast?.type === 'module' ? oscA.ast.memoryDeclarationLines.map(line => line.arguments[0].value) : []
		).toEqual(['phase', 'frequency', 'localA']);
	});

	test('rejects unknown prototype shapes', () => {
		expect(() =>
			compileFixtureProgramSource(`
8f4e/v1
...
entryEnd
`)
		).toThrow('Undeclared identifier: missing');
	});

	test('rejects executable instructions inside prototype blocks', () => {
		expect(() =>
			compileFixtureProgramSource(`
8f4e/v1
...
prototype invalidShape
push 1
prototypeEnd
`)
		).toThrow('This instruction is not allowed inside this block type');
	});
