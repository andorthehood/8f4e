...
- Updated TODO 205 to add explicit anti-patterns, validation checkpoints, and "no host folder move" wording.
- Required callback injection for `getCodeBuffer/getMemory` and forbade runtime package imports from `@8f4e/editor`.
- Added host vs worker entrypoint split and multi-entry build requirement (with main-thread runtime exception).
- Updated TODO template to include Anti-Patterns and Validation Checkpoints sections.
