...
import { collectNamespacesFromASTs, collectFunctionIdsFromAsts } from './semantic/buildNamespace';
import {
	AST,
	CompileOptions,
	CompiledModule,
...
export { prepassNamespace, collectNamespacesFromASTs, collectFunctionIdsFromAsts } from './semantic/buildNamespace';
...
export default function compile(
	...
	// Collect function IDs as stubs before full compilation so `call` target validation
	// during semantic normalization can run before function codegen completes.
	const functionStubs = collectFunctionIdsFromAsts(astFunctions);
	...
	const compiledFunctions = astFunctions.map((ast, index) =>
		compileFunction(ast, namespaces, EXPORTED_FUNCTION_COUNT + index, functionTypeRegistry, functionStubs)
	);
